...
const benefitIcons: IconName[] = ["rocket", "spark", "gift", "star"];

export function LandingCreatorBenefits() {
  const { t } = useLocale();
  ...
  return (
    <Section>
      <LandingScrollReveal>
        <SectionHeader
          title={t("landing.creatorBenefits.title")}
          description={t("landing.creatorBenefits.subtitle")}
        />
      </LandingScrollReveal>
        ...
        {keys.map((key, i) => (
          <LandingScrollReveal key={key} delay={i * 0.06}>
            ...
            >
              ...
              <span>{t(`landing.creatorBenefits.${key}`)}</span>
            </motion.li>
          </LandingScrollReveal>
        ))}
      </ul>
      <Button href="/login" className="min-h-11">
        {t("landing.creatorBenefits.cta")}
      </Button>
    </Section>
  );
}
